...
\item Evaluate trade-offs between client-side and server-side technologies for behavioral research
...
\item<5-> run a study on MTurk or Crowdflower, with YouGov, or another online platform?
\end{itemize}
}
...
Combining SRS and experimental design


Says nothing about heterogeneity -> ATE is not necessarily an effect for any given unit in the population -> it is an average of individual-level effects

Imagine two CATES (conditional average treatment effects)
...
\frametitle{Some considerations\dots}
...
\item<3-> Random sampling ensures that samples are, \textit{in expectation}, representative of the population \textit{in all respects}
	...
	\end{itemize}
\item<4-> Only works in a world of perfect coverage and zero response bias
	\begin{itemize}
	\item Says nothing about response rate!
	\end{itemize}
\end{itemize}
...
\onslide<7->{Which of these matter? Discuss with the person next to you.}
...
\frame{
\begin{center}
\includegraphics[width=1.1\textheight]{images/mullinix1}
\end{center}
}
...
\item<2-> If you think there is heterogeneity, then we probably do not care about the SATE anyway
	...
	\end{itemize}
\item<3-> Two basic analytic strategies
	\begin{itemize}
...
\frame{
Does this mean all convenience samples are created equal?
...
\item Google Spreadsheet Forms:\\
\url{https://www.google.co.uk/forms/about/}
%\item These are interesting because you can hack them
\end{itemize}
}
...
% tecnical pretesting (does it work?)
% substantive pretesting (does it make sense?)
...
\item Best workflow is lab-like:
	...
	\end{enumerate}
\item So, think of the following as relevant the first two steps in that process
...
\item<3-> Purely model-based, so only as good as the model
	...
	\end{itemize}
\item<4-> Not well-tested on experimental data
\end{itemize}
